# LAYER 5 — LANDING ZONE: QA SPOKE
# Purpose: deploys the QA spoke VNet, peers it to the Hub, and adds
# a UDR that routes all traffic through the Hub Firewall.
# This pattern is identical for Dev and Prod — only variable values differ.
...
terraform {
  ...
  backend "azurerm" {
    ...
    container_name       = "landing-zones-qa"
    key                  = "landing-zones-qa.tfstate"
  }
}
...
resource "azurerm_network_security_group" "spoke" {
  ...
  security_rule {
    ...
    destination_address_prefix = "10.2.0.0/16"   # this spoke range
    source_port_range          = "*"
...
# ── RBAC — QA team gets Contributor on this subscription only ────────────────
# resource "azurerm_role_assignment" "qa_team_contributor" {
#   scope                = "/subscriptions/${var.qa_subscription_id}"
#   role_definition_name = "Contributor"
#   principal_id         = var.qa_team_group_object_id
# }

# # QA team gets Reader on the Hub VNet (to view, not modify)
# resource "azurerm_role_assignment" "qa_team_hub_reader" {
