# LAYER 5 — LANDING ZONE: PROD SPOKE
# Purpose: deploys the Prod spoke VNet, peers it to the Hub, and adds
# a UDR that routes all traffic through the Hub Firewall.
# This pattern is identical for Dev and QA — only variable values differ.
...
terraform {
  ...
  backend "azurerm" {
    ...
    container_name       = "landing-zones-prod"
    key                  = "landing-zones-prod.tfstate"
  }
}
...
resource "azurerm_network_security_group" "spoke" {
  ...
  security_rule {
    ...
    destination_address_prefix = "10.3.0.0/16"   # this spoke range
    source_port_range          = "*"
...
# ── RBAC — Prod team gets Contributor on this subscription only ──────────────
# resource "azurerm_role_assignment" "prod_team_contributor" {
#   scope                = "/subscriptions/${var.prod_subscription_id}"
#   role_definition_name = "Contributor"
#   principal_id         = var.prod_team_group_object_id
# }

# # Prod team gets Reader on the Hub VNet (to view, not modify)
# resource "azurerm_role_assignment" "prod_team_hub_reader" {
